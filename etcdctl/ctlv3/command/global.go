...
type GlobalFlags struct {
    Insecure              bool  // 关闭传输层安全
    InsecureSkipVerify    bool  // 关闭对服务端做的证书校验
    InsecureDiscovery     bool  // 关闭服务
    Endpoints             []string          // 终端列表
    DialTimeout           time.Duration     // ping 超时
    CommandTimeOut        time.Duration     // 命令超时
    KeepAliveTime         time.Duration     // 保活时间
    KeepAliveTimeout      time.Duration     // 保活超时
    DNSClusterServiceName string    // DNS 服务名

    TLS transport.TLSInfo   // 证书、Key、CA、服务名

    OutputFormat string     // 输出格式，json、table等
    IsHex        bool       // 字节字符串，使用十六进制

    User     string     // 用户名
    Password string     // 密码

    Debug bool  // 客户端 debug
}
